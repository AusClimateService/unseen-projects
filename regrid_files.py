...
def regrid_dataset(var, files, outfile, dx=0.5, regrid_method="conservative"):
    ...
    Examples
    --------
    >>> python regrid_files.py tmax tmax_AGCD-CSIRO_r005**.nc tmax_AGCD-CSIRO_r05_1901-2024.nc 0.5 conservative
    ...
    ds_regrid.attrs["history"] = (
        f"{datetime.datetime.now().strftime('%a %b %d %H:%M:%S %Y')}: {regrid_method} regrid to {dx}x{dx} degrees, /g/data/xv83/unseen-projects/code/regrid_files.py\n"
        + ds_regrid.attrs["history"]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("-r", "--resolution", type=float, default=0.5, help="Target grid (degrees)")
    parser.add_argument("--regrid_method", type=str, default="conservative", help="Regrid method")
