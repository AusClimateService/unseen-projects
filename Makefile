...
GEV_STATIONARY=${PROJECT_DIR}/data/gev_params_stationary_${METRIC}_${MODEL}-${EXPERIMENT}_${BASE_PERIOD_TEXT}_${TIMESCALE}_${REGION}.nc
GEV_STATIONARY_ADDITIVE_BC=${PROJECT_DIR}/data/gev_params_stationary_${METRIC}_${MODEL}-${EXPERIMENT}_${BASE_PERIOD_TEXT}_${TIMESCALE}_${REGION}_bias-corrected-${OBS_DATASET}-additive.nc
GEV_STATIONARY_MULTIPLICATIVE_BC=${PROJECT_DIR}/data/gev_params_stationary_${METRIC}_${MODEL}-${EXPERIMENT}_${BASE_PERIOD_TEXT}_${TIMESCALE}_${REGION}_bias-corrected-${OBS_DATASET}-multiplicative.nc
GEV_NONSTATIONARY_OBS=${PROJECT_DIR}/data/gev_params_nonstationary_${GEV_BEST_MODEL_TEST}_${METRIC}_${OBS_DATASET}_${OBS_TIME_PERIOD}_${TIMESCALE}_${REGION}.nc
GEV_NONSTATIONARY=${PROJECT_DIR}/data/gev_params_nonstationary_${GEV_BEST_MODEL_TEST}_${METRIC}_${MODEL}-${EXPERIMENT}_${BASE_PERIOD_TEXT}_${TIMESCALE}_${REGION}.nc
GEV_NONSTATIONARY_ADDITIVE_BC=${PROJECT_DIR}/data/gev_params_nonstationary_${GEV_BEST_MODEL_TEST}_${METRIC}_${MODEL}-${EXPERIMENT}_${BASE_PERIOD_TEXT}_${TIMESCALE}_${REGION}_bias-corrected-${OBS_DATASET}-additive.nc
GEV_NONSTATIONARY_MULTIPLICATIVE_BC=${PROJECT_DIR}/data/gev_params_nonstationary_${GEV_BEST_MODEL_TEST}_${METRIC}_${MODEL}-${EXPERIMENT}_${BASE_PERIOD_TEXT}_${TIMESCALE}_${REGION}_bias-corrected-${OBS_DATASET}-multiplicative.nc
...
${METRIC_FCST} : ${FCST_DATA}
	@if [ ! -f $@ ]; then \
		...
		echo "${FILEIO} $< $@ --forecast ${METRIC_OPTIONS} ${METRIC_OPTIONS_FCST} ${FCST_IO_OPTIONS}" >> job_${MODEL}.sh
		qsub job_${MODEL}.sh; \
	fi

## independence-test : independence test for different lead times # todo: Update analysis for file
independence-test : ${INDEPENDENCE_FILE}
${INDEPENDENCE_FILE} : ${METRIC_FCST}
	@if [ ! -f $@ ]; then \
		cp job_independence.sh job_independence_${MODEL}.sh
		echo "${INDEPENDENCE} $< ${VAR} $@ ${INDEPENDENCE_OPTIONS}" >> job_independence_${MODEL}.sh
		qsub job_independence_${MODEL}.sh; \
	fi
...
${GEV_STATIONARY_OBS} : ${METRIC_FCST}
...
${GEV_NONSTATIONARY_OBS} : ${METRIC_FCST} ${METRIC_OBS}
...
${GEV_NONSTATIONARY} : ${METRIC_FCST} ${METRIC_OBS}
...
gev-params : gev-params-stationary gev-params-nonstationary gev-params-stationary-additive-bias gev-params-nonstationary-additive-bias gev-params-stationary-multiplicative-bias gev-params-nonstationary-multiplicative-bias
...
## prereq files (todo: move back): ${METRIC_OBS} ${METRIC_FCST} ${METRIC_FCST_ADDITIVE_BC} ${METRIC_FCST_MULTIPLICATIVE_BC} ${INDEPENDENCE_FILE} ${SIMILARITY_FILE} ${SIMILARITY_ADDITIVE_BC_FILE} ${SIMILARITY_MULTIPLICATIVE_BC_FILE} ${GEV_STATIONARY} ${GEV_NONSTATIONARY} ${GEV_STATIONARY_ADDITIVE_BC} ${GEV_NONSTATIONARY_ADDITIVE_BC} ${GEV_STATIONARY_MULTIPLICATIVE_BC} ${GEV_NONSTATIONARY_MULTIPLICATIVE_BC} ${FCST_DATA} 
...
${NOTEBOOK_OUT_DIR}/spatial_analysis_${METRIC}_${MODEL}.ipynb : ${NOTEBOOK_IN_DIR}/spatial_analysis.ipynb
	${PAPERMILL} $< $@ -p model_name ${MODEL} -p metric ${METRIC} -p var ${VAR} -p time_freq ${TIME_FREQ} -p reference_time_period '${REFERENCE_TIME_PERIOD}' -p time_agg ${TIME_AGG} -p covariate_base ${GEV_COVARIATE_BASE} -p gev_trend_period ${GEV_TREND_PERIOD} -p plot_dict ${PLOT_DICT} -p fig_dir ${FIG_DIR} -p bias_correction ${BIAS_CORRECTION} -p plot_additive_bc ${PLOT_ADDITIVE_BC} -p plot_multiplicative_bc ${PLOT_MULTIPLICATIVE_BC} -p shapefile ${SHAPEFILE} -p shape_overlap ${SHAPE_OVERLAP} -p obs_file ${METRIC_OBS} -p model_file ${METRIC_FCST} -p model_add_bc_file ${METRIC_FCST_ADDITIVE_BC} -p model_mulc_bc_file ${METRIC_FCST_MULTIPLICATIVE_BC} -p independence_file ${INDEPENDENCE_FILE} -p independence_plot ${INDEPENDENCE_PLOT} -p min_lead_spatial_agg ${MIN_LEAD_SHAPE_SPATIAL_AGG} -p similarity_raw_file ${SIMILARITY_FILE} -p similarity_add_bc_file ${SIMILARITY_ADDITIVE_BC_FILE} -p similarity_mulc_bc_file ${SIMILARITY_MULTIPLICATIVE_BC_FILE} -p similarity_raw_plot ${SIMILARITY_PLOT} -p similarity_add_bc_plot ${SIMILARITY_ADDITIVE_BC_PLOT} -p similarity_mulc_bc_plot ${SIMILARITY_MULTIPLICATIVE_BC_PLOT} -p gev_params_stationary_file ${GEV_STATIONARY} -p gev_params_nonstationary_file ${GEV_NONSTATIONARY} -p gev_params_stationary_add_bc_file ${GEV_STATIONARY_ADDITIVE_BC} -p gev_params_nonstationary_add_bc_file ${GEV_NONSTATIONARY_ADDITIVE_BC} -p gev_params_stationary_mulc_bc_file ${GEV_STATIONARY_MULTIPLICATIVE_BC} -p gev_params_nonstationary_mulc_bc_file ${GEV_NONSTATIONARY_MULTIPLICATIVE_BC}

metric-obs-spatial-analysis : ${NOTEBOOK_OUT_DIR}/spatial_analysis_${METRIC}_${OBS_DATASET}.ipynb
${NOTEBOOK_OUT_DIR}/spatial_analysis_${METRIC}_${OBS_DATASET}.ipynb : ${NOTEBOOK_IN_DIR}/spatial_obs.ipynb
	${PAPERMILL} $< $@ -p obs_name ${OBS_LABEL} -p metric ${METRIC} -p var ${VAR} -p reference_time_period '${REFERENCE_TIME_PERIOD}' -p time_freq ${TIME_FREQ} -p time_agg ${TIME_AGG} -p covariate_base ${GEV_COVARIATE_BASE} -p gev_trend_period ${GEV_TREND_PERIOD} -p plot_dict ${PLOT_DICT} -p fig_dir ${FIG_DIR} -p obs_file ${METRIC_OBS} -p gev_params_stationary_file ${GEV_STATIONARY_OBS} -p gev_params_nonstationary_file ${GEV_NONSTATIONARY_OBS} -p shapefile ${SHAPEFILE} -p shape_overlap ${SHAPE_OVERLAP}
